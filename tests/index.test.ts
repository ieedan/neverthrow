...
describe("Result.Ok", () => {
	...
	describe("orElse", () => {
		it("Skips orElse on an Ok value", () => {
			const okVal = ok(12);
			const errorCallback = vitest.fn((_errVal) =>
...
describe("ResultAsync", () => {
	...
	describe("map", () => {
		...
		it("Maps a value using an asynchronous function", async () => {
			...
			const mapAsyncFn = vitest.fn((number) =>
	...
	describe("mapErr", () => {
		...
		it("Maps an error using an asynchronous function", async () => {
			...
			const mapErrAsyncFn = vitest.fn((str) =>
				Promise.resolve(`Error: ${str}`),
			);
	...
	describe("ResultAsync.fromThrowable", () => {
		...
		it("handles synchronous errors", async () => {
			const example = ResultAsync.fromThrowable(() => {
				// biome-ignore lint/correctness/noConstantCondition: it's fine
